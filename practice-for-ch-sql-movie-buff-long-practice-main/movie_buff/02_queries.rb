...
def cast_list(title)
  ...
  Actor.select(:id, :name).joins(:movies).order( ord: :asc ).where("movies.title = ?", title)
...
def vanity_projects
  ...
  Movie.joins(:actors).where("movies.director_id = actors.id").select(:id, :title, "actors.name").where("castings.ord = ?", 1)
